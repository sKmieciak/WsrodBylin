...
export default function CartItem({ item, updateItem, removeItem }: CartItemProps) {
  ...
  const fullImageUrl = item.productImage.startsWith("http")
    ? item.productImage
  ...
  return (
    <div className="flex items-center gap-4 p-3 bg-white rounded-xl shadow-sm border">
      <img
        src={fullImageUrl || "/images/placeholder.svg"}
      ...
      <div className="flex flex-col flex-1">
        <p className="font-semibold text-sm md:text-base leading-tight break-words">
          {item.productName}
        </p>
